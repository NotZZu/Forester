...
public class ObjectPool : MonoBehaviour
{
    ...
    void Setting()
    {
        ...
        {
            CreatePool(itemData, 100);
        }
    }

    void CreatePool(ItemData itemData, int count)
    {
        List<GameObject> itemList = new List<GameObject>();
        for (int i = 0; i < count; i++)
        {
            ...
            ItemInfo itemInfo = item.GetComponent<ItemInfo>();
            //itemInfo.SetProperties(itemData.sprite, 1, itemData.itemName, itemData.damage, itemData.weight , itemData.properties);
            itemInfo.SetProperties(itemData);
            item.GetComponent<SpriteRenderer>().sprite = itemData.sprite;
        }
        itemPools[itemData.itemName] = itemList;
    }
